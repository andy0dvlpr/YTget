...
namespace YTget
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            bool audioplchecked = audioPLCheckBox.Checked;
            bool videochecked = videoRadio.Checked;
            bool videoplchecked = videoPLCheckBox.Checked;
            ...
            if (audiochecked)
            {
                ...
                if (audioplchecked)
                {
                    RunCMD($"youtube-dl.exe --yes-playlist -x --audio-format mp3 --audio-quality {bitrate}k -o \"{saveLocation}\\{fileName}s.%(ext)s\" \"{ytURL}\"");
                }
            }
            if (videochecked)
            {
                if (!videoplchecked)
                {
                    RunCMD($"youtube-dl.exe -f mp4 -o \"{saveLocation}\\{fileName}s.%(ext)s\" \"{ytURL}\"");
                }
                if (videoplchecked)
                {
                    RunCMD($"youtube-dl.exe --yes-playlist -f mp4 -o \"{saveLocation}\\{fileName}s.%(ext)s\" \"{ytURL}\"");
                }
            }
        }
    }
